...
CONFIG_FILES = [USER_PREFS_DIR / s for s in os.listdir(USER_PREFS_DIR)]
...
if RUNNING_CLI:
    if ARGS_FILE.exists():
        ARGS = parse_arguments(ARGS_FILE, PROGRAM, VERSION, DESCRIPTION, EPILOG)
...
class Settings():
    def __init__(self, piped=INPUT, args=ARGS, config=CONFIG):
        self._input = piped
        self.args = args
        self.env = ENV
        self.config = config
    ...
    def __getitem__(self, key):
        ...
        if hasattr(self.args, "__dict__") and key in self.args.__dict__:
            return self.args.__dict__[key]
